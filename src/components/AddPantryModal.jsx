  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="sheet sup" style={{ padding: 24 }} onClick={e => e.stopPropagation()}>
        ...
        <input placeholder="Name (z.B. Olivenöl)" value={name} onChange={e => setName(e.target.value)}
          style={{ width: "100%", padding: "10px 14px", fontSize: 14, marginBottom: 10, borderRadius: 10 }}/>
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          <input placeholder="Menge" value={qty} onChange={e => setQty(e.target.value)} type="number"
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button className="btn"
            ...
            style={{ flex: 1, padding: 13, borderRadius: 12, background: "var(--ac)", color: "#fff", fontSize: 14, fontWeight: "bold" }}>Hinzufügen</button>
        </div>
      </div>
    </div>
  );
